from utils.functions import  fed_avg, get_masked_model_chatgpt, SimpleMLP4, save_file,  open_file, batch_data_non_iid, set_model_weights, get_cropped_model_chatgpt, group_gradient, group_hessian_new, norm_grad
...
for i in range(100):
    clients_batched[i] = batch_data_non_iid(client_data[i], client_labels[i], bs=32)
...
for i in range(epochs):
    ...
    for a in range(20):
        ...
        if i % 5 == 0 and i > 0:
            score1 = model1.evaluate(clients_batched[client_names[a]], verbose=1)
    ...
    for a in range(20,40):
        ...
        if i % 5 == 0 and i > 0:
            score2 = model2.evaluate(clients_batched[client_names[a]], verbose=1)
    ...
    for a in range(40,60):
        ...
        if i % 5 == 0 and i > 0:
            score3 = model3.evaluate(clients_batched[client_names[a]], verbose=1)
    ...
    if i%5==0 and i>0:
        sample_list = [global_accuracy, global_loss, group1_accuracy, group1_loss, group2_accuracy, group2_loss,
                       ...
                       group3_train_accuracy, group3_train_loss, group4_train_accuracy, group4_train_loss,group5_train_accuracy, group5_train_loss,global_weight]
        # save_file_name= f'../Data/non iid/test/Fair_Cifar10_non_iid_zone_{q}_{qm}_{epochs}_{lr}.pkl'
        save_file_name = f'../Data/non iid/test/test lr/Fair_Cifar10_non_iid_zone_{q}_{qm1}_{qm2}_{qm3}_{qm4}_{qm5}_{epochs}_{lr}.pkl'
