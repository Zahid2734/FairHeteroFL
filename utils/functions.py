...
def batch_data_new(data_shard, bs=32, test_proportion=0.2):
    '''Takes in a client's data shard and creates a tfds object off it
    args:
        data_shard: a list of tuples (data, label) representing a client's data shard
        bs: batch size
        test_proportion: proportion of data to use for testing (default is 0.2)
    return:
        train_data: tf.data.Dataset object for training data
        test_data: tf.data.Dataset object for testing data'''
    # Separate shard into data and labels lists
    data, label = zip(*data_shard)
    label = tf.keras.utils.to_categorical(label, 10)

    # Split data into training and testing sets
    train_data, test_data = train_test_split(list(zip(data, label)),
    ...
    train_data = tf.data.Dataset.from_tensor_slices(train_data).shuffle(len(train_data)).batch(bs)
    test_data = tf.data.Dataset.from_tensor_slices(test_data).shuffle(len(test_data)).batch(bs)
...
def group_hessian(gradlist, loss_list, group_num, q,qm):
    gm=1/group_num
    nm=len(gradlist)
    pm=1/nm
    lr = 0.01
