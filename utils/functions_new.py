...
def slicing_1(slicing_range, sla, axis=0):
  if len(slicing_range)==1:
    sla= sla[slicing_range[0][0]:slicing_range[0][1]]
  else:
    sla= np.concatenate((sla[slicing_range[0][0]:slicing_range[0][1]], sla[slicing_range[1][0]:slicing_range[1][1]]), axis=axis)
  return sla

def slicing_2(slicing_range, sla, axis=1):
  if len(slicing_range)==1:
    sla= sla[:,slicing_range[0][0]:slicing_range[0][1]]
  else:
    sla= np.concatenate((sla[:,slicing_range[0][0]:slicing_range[0][1]], sla[:,slicing_range[1][0]:slicing_range[1][1]]), axis=axis)
...
def slicing_3(slicing_range, sla, axis=2):
  if len(slicing_range)==1:
    sla= sla[:,:,slicing_range[0][0]:slicing_range[0][1]]
  else:
    sla= np.concatenate((sla[:,:,slicing_range[0][0]:slicing_range[0][1]], sla[:,:,slicing_range[1][0]:slicing_range[1][1]]), axis=axis)
...
def slicing_4(slicing_range, sla, axis=3):
  if len(slicing_range)==1:
    sla= sla[:,:,:,slicing_range[0][0]:slicing_range[0][1]]
  else:
    sla= np.concatenate((sla[:,:,:,slicing_range[0][0]:slicing_range[0][1]], sla[:,:,:,slicing_range[1][0]:slicing_range[1][1]]), axis=axis)
...
def get_smaller_weight(bigger_model, slicing_ranges):
  sla = bigger_model

  if len(slicing_ranges)==1:
    sla= slicing_1(slicing_ranges[0], sla, axis=0)
  elif len(slicing_ranges)==2:
    sla= slicing_1(slicing_ranges[0], sla, axis=0)
    sla= slicing_2(slicing_ranges[1], sla, axis=1)
  elif len(slicing_ranges)==3:
    sla= slicing_1(slicing_ranges[0], sla, axis=0)
    sla= slicing_2(slicing_ranges[1], sla, axis=1)
    sla= slicing_3(slicing_ranges[2], sla, axis=2)

  elif len(slicing_ranges)==4:
    sla= slicing_1(slicing_ranges[0], sla, axis=0)
    sla= slicing_2(slicing_ranges[1], sla, axis=1)
    sla= slicing_3(slicing_ranges[2], sla, axis=2)
    sla= slicing_4(slicing_ranges[3], sla, axis=3)

  return sla

import numpy as np

def crop_weight_indices(bigger_weight, smaller_weight, start_index):
    """
    Crops the bigger weight to the shape of the smaller weight starting from a specific index with circular indexing.
    If the end index exceeds the dimension of the bigger weight, fill the rest from the beginning index.
    """
    # Get the shape of the smaller weight
    bigger_weight = np.array(bigger_weight)
    bigger_shape= bigger_weight.shape
    smaller_weight = np.array(smaller_weight)
    smaller_shape = smaller_weight.shape
    slice_indices=[]
    ...
    for i in range(len(smaller_shape)):
      if bigger_shape[i]== smaller_shape[i]:
        slice_indices.append([(0,smaller_shape[i])])
      else:
        # Calculate the corrected start index using modulo to wrap around
        corrected_start_index = start_index % bigger_shape[i]

        # Calculate the corrected end index using modulo to wrap around and fill from the beginning
        corrected_end_index = (start_index + smaller_shape[i]) % bigger_shape[i]
        print(corrected_start_index, corrected_end_index)
        # Check if the corrected start index is greater than the corrected end index
        if corrected_start_index > corrected_end_index:
            # Create two slices to cover the range from corrected start index to the end and from the beginning to corrected end index
            slice_indices.append([(corrected_start_index, bigger_weight.shape[i]), (0, corrected_end_index)])

        else:
            # Create a single slice from corrected start index to corrected end index
            slice_indices.append([(corrected_start_index, corrected_end_index)])
    return slice_indices

def get_cropped_model_fedrolex(large_model, small_model, start_index):
    new_model = []
    new_indices=[]
    for i in range(len(small_model)):
        indices= crop_weight_indices(large_model[i], small_model[i], start_index)
        new_indices.append(indices)
        new_model.append(get_smaller_weight(large_model[i], indices))

    return new_model, new_indices

def put_bigger_model(bigger_model, smaller_model, indices):
    bla= np.array(bigger_model)
    smaller_model=np.array(smaller_model)
    bla = bla * 0
    for i in range(len(smaller_model)):
        if len(indices[i])==1:
            bla[i]=put_dim1(bla[i],smaller_model[i],indices[i])
        elif len(indices[i])==2:
            bla[i]=put_dim2(bla[i],smaller_model[i],indices[i])
        elif len(indices[i])==3:
            bla[i]=put_dim3(bla[i],smaller_model[i],indices[i])
        elif len(indices[i])==4:
            bla[i]=put_dim4(bla[i],smaller_model[i],indices[i])
    return bla

def put_dim1(bigger_model,smaller_model,slicing_ranges):
  for i in range(len(slicing_ranges[0])):
    if len(slicing_ranges[0])==1:
      starti=slicing_ranges[0][0][0]
      endi= slicing_ranges[0][0][1]
      sstarti=0
      sendi= slicing_ranges[0][0][1]-slicing_ranges[0][0][0]
    else:
      if i==0:
        starti=slicing_ranges[0][0][0]
        endi= slicing_ranges[0][0][1]
        ...
        starti=slicing_ranges[0][1][0]
        endi= slicing_ranges[0][1][1]
        sstarti=slicing_ranges[0][0][1]-slicing_ranges[0][0][0]
        sendi= slicing_ranges[0][1][1]+sstarti

    bigger_model[starti:endi] =smaller_model[sstarti:sendi]
  return bigger_model

def put_dim2(bigger_model,smaller_model,slicing_ranges):
  for i in range(len(slicing_ranges[0])):
    if len(slicing_ranges[0])==1:
      starti=slicing_ranges[0][0][0]
      endi= slicing_ranges[0][0][1]
    ...
    for j in range(len(slicing_ranges[1])):
      if len(slicing_ranges[1])==1:
        startj=slicing_ranges[1][0][0]
        endj= slicing_ranges[1][0][1]
        sstartj=0
        sendj= slicing_ranges[1][0][1]-slicing_ranges[1][0][0]
      else:
        if j==0:
          startj=slicing_ranges[1][0][0]
          endj= slicing_ranges[1][0][1]
          ...
          startj=slicing_ranges[1][1][0]
          endj= slicing_ranges[1][1][1]
          sstartj=slicing_ranges[1][0][1]-slicing_ranges[1][0][0]
          sendj= slicing_ranges[1][1][1]+sstartj
      bigger_model[starti:endi,startj:endj] =smaller_model[sstarti:sendi, sstartj:sendj]
  return bigger_model

def put_dim3(bigger_model,smaller_model,slicing_ranges):
  for i in range(len(slicing_ranges[0])):
    if len(slicing_ranges[0])==1:
      starti=slicing_ranges[0][0][0]
      endi= slicing_ranges[0][0][1]
    ...
    for j in range(len(slicing_ranges[1])):
      if len(slicing_ranges[1])==1:
        ...
        if j==0:
          ...
          sstartj=slicing_ranges[1][0][1]-slicing_ranges[1][0][0]
          sendj= slicing_ranges[1][1][1]+sstartj
      for k in range(len(slicing_ranges[2])):
        if len(slicing_ranges[2])==1:
          startk=slicing_ranges[2][0][0]
          endk= slicing_ranges[2][0][1]
          sstartk=0
          sendk= slicing_ranges[2][0][1]-slicing_ranges[2][0][0]
        else:
          if k==0:
            startk=slicing_ranges[2][0][0]
            endk= slicing_ranges[2][0][1]
            ...
            startk=slicing_ranges[2][1][0]
            endk= slicing_ranges[2][1][1]
            sstartk=slicing_ranges[2][0][1]-slicing_ranges[2][0][0]
            sendk= slicing_ranges[2][1][1]+sstartk
        bigger_model[starti:endi,startj:endj, startk:endk] =smaller_model[sstarti:sendi, sstartj:sendj, sstartk:sendk]
  return bigger_model

def put_dim4(bigger_model,smaller_model,slicing_ranges):
  for i in range(len(slicing_ranges[0])):
    if len(slicing_ranges[0])==1:
      starti=slicing_ranges[0][0][0]
      endi= slicing_ranges[0][0][1]
      ...
      for k in range(len(slicing_ranges[2])):
        if len(slicing_ranges[2])==1:
          ...
          if k==0:
            ...
            sstartk=slicing_ranges[2][0][1]-slicing_ranges[2][0][0]
            sendk= slicing_ranges[2][1][1]+sstartk
        for m in range(len(slicing_ranges[3])):
          if len(slicing_ranges[3])==1:
            startm=slicing_ranges[3][0][0]
            endm= slicing_ranges[3][0][1]
            sstartm=0
            sendm= slicing_ranges[3][0][1]-slicing_ranges[3][0][0]
          else:
            if m==0:
              startm=slicing_ranges[3][0][0]
              endm= slicing_ranges[3][0][1]
              ...
              startm=slicing_ranges[3][1][0]
              endm= slicing_ranges[3][1][1]
              sstartm=slicing_ranges[3][0][1]-slicing_ranges[3][0][0]
              sendm= slicing_ranges[3][1][1]+sstartm
          print(startm, endm,sstartm, sendm)
          bigger_model[starti:endi,startj:endj, startk:endk, startm:endm] =smaller_model[sstarti:sendi, sstartj:sendj, sstartk:sendk, sstartm:sendm]
  return bigger_model